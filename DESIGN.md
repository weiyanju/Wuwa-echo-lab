...
typography:
  ...
  caption:
    ...
    letterSpacing: "0"
  data:
    fontFamily: "IBM Plex Sans SC, system-ui, sans-serif"
    fontSize: "1.125rem"
    fontWeight: 600
    lineHeight: 1.05
    letterSpacing: "0"
...
暗色主题中的低对比度渐变和宽柔阴影属于默认允许的主题层级表达，不需要逐项登记为项目例外；它们仍须保持文字可读、状态语义稳定，并避免霓虹发光、高对比装饰和多层阴影堆叠。
...
- 浅色模式以冷灰边框与近白表面建立结构；暗色模式允许使用低对比度渐变和宽柔阴影辅助区分表面层级。
...
暗色模式使用 `#0f1720` 画布、`#17232d` 次级表面、`#e7eef4` 主文字、`#a9bac7` 次级文字、`#2c3c48` 强边界和 `#243542` 柔边界。主蓝映射为 `#5da8ff`，但视觉权重、状态含义和组件结构必须与浅色模式相同。暗色静态表面默认允许低对比度渐变和宽柔阴影，用于维持当前已批准的表面层级；无需逐项登记例外，但禁止霓虹发光、高对比装饰渐变、多层阴影堆叠或另一套终端产品语言。
...
**The Light-First Rule.** 浅色模式是基准，暗色模式做等价语义映射，并可默认使用低对比度渐变和宽柔阴影辅助表面分层；它不能成为霓虹化或开发者控制台风格的另一套产品。
...
活动 CSS 使用与本文件一致的 canonical token：`--ink-strong`、`--ink-main`、`--text-muted`、`--decorative-muted`、`--border-strong` 和 `--border-soft`。迁移期继续提供以下等值兼容别名，但新增或修改样式不得继续引用旧名称：

| 兼容别名 | Canonical token |
|---|---|
| `--ink-deep` | `--ink-strong` |
| `--ink` | `--ink-main` |
| `--steel` | `--text-muted` |
...
`--charcoal` 目前没有可证明的单一 canonical 对应关系，继续作为现有中性文字 token 保留；只有在语义拆分和最终色值都明确后才迁移。

### Approved functional graphic exceptions

- Bayes Exact / Wildcard 路径左侧的实线与虚线路径标记属于推理结构图形。
- 登录标题的 2px 输出指示条及其 `0.14em` 间距属于品牌交互图形；它只表达逐字输出、标点停顿与完成归档，不承担持续活动提示。
...
系统以结构化扁平策略为浅色基准。浅色静态表面通过背景色和 1px 冷灰边框分层；顶部粘性导航、菜单和可拖动历史面板使用阴影。暗色静态表面默认允许低对比度渐变和宽柔阴影辅助层级，不需要登记为项目例外。焦点轮廓属于可访问性反馈，不是装饰阴影。
...
**The Flat-by-Default Rule.** 浅色静态面板和卡片保持平坦；如果移除阴影后层级仍清楚，就不允许添加阴影。暗色主题按已批准基线默认允许低对比度渐变和宽柔阴影。

**The One Depth Cue Rule.** 浅色模式下，同一个元素只能选择清晰边框或宽柔阴影作为主要层级线索，禁止 1px 边框与 16px 以上模糊阴影的 ghost-card 组合。暗色主题可以同时使用低对比度边框和宽柔阴影，但不得叠加发光边缘或多层装饰阴影。

**The Structural Shadow Rule.** 浅色阴影只说明粘性、悬浮、拖动或聚焦关系；暗色表面可以用单层宽柔阴影表达层级。两种主题都禁止用重阴影、发光边缘和多层阴影制造“高级感”。
...
- 浅色静态卡片无阴影；暗色静态表面可以使用主题允许的低对比度渐变和宽柔阴影。不允许卡片套卡片。大面板内边距 24–28px，中型面板 16–20px，密集区域 8–12px。
...
- “创建档案”是可恢复的开户入口：系统账户已创建但尚无任何绑定 UID 时，使用正确凭据再次提交会继续进入同一登录卡片内的 UID 初始化页；密码错误、账户停用或已有绑定 UID 时保持在认证表单。该恢复不新增页面、卡片或普通登录捷径。
- 首次 UID 绑定页使用与认证页活动页签相同排版角色和垂直节奏的全宽单抬头“绑定 UID”：文字居中、底部为深色 1px 规则线，但不提供点击、页签或步骤条语义，也不再在内容区重复大标题。说明、字段和提示保持同一左基线。标题区不放置返回图标；“返回登录”作为蓝色绑定主按钮下方的居中灰色文字动作，保留至少 44px 点击高度、可见焦点和 busy 禁用状态。该动作退出当前系统账号后返回认证页，不表示普通“上一步”。
- 标题输出指示器是功能性状态图形，不属于彩色侧边条：逐字输出时保持稳定 2px 竖条，中文逗号处短暂压缩，末字后收束为点并淡出；完成页面保持静态，禁止无限闪烁。`prefers-reduced-motion`、520px 及以下窄屏、后台页面、字体准备失败或调度失败时直接显示完整标题和认证卡片，不渲染指示器。
...
- 常规状态反馈为 80–120ms，面板显隐为 120–160ms；首页一次性标题归档交接允许认证卡片使用 260ms 入场。只动画 opacity、transform 或颜色等不会触发布局重排的属性。
...
- **Don't** 使用大面积紫色、蓝紫 AI 渐变、渐变文字、玻璃拟态、发光边缘、装饰性卡片堆叠或超大 hero；暗色主题允许的低对比度表面渐变不属于此类装饰渐变。
- **Don't** 在卡片、列表、callout 或 alert 左右使用超过 1px 的彩色侧边强调；功能图形必须真实表达路径、进度或结构。
- **Don't** 在浅色模式组合 1px 边框和 16px 以上模糊阴影制造 ghost card，也不要使用嵌套卡片；暗色主题可以按默认规则使用低对比度边框和宽柔阴影。
...
- **Don't** 把已登记的 Bayes 路径、登录输出指示器或其他功能图形当作装饰性侧边条删除，也不要把一次性归档指示器改回无限闪烁光标。
