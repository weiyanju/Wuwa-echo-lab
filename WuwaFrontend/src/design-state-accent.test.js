...
test('model judgement surfaces inherit the visible disclosure row state', async () => {
  ...
  assert.match(darkSummary, /var\(--model-surface-accent,\s*#5da8ff\)\s+16%,\s*var\(--border-soft\)/)
  assert.match(darkSummary, /var\(--model-surface-accent,\s*#5da8ff\)\s+6%,\s*var\(--surface-soft\)/)
  assert.match(darkLabel, /var\(--model-surface-accent,\s*#5da8ff\)\s+46%,\s*var\(--ink-strong\)/)
...
test('evaluation dark theme consumes existing semantic theme tokens without remapping values', async () => {
  ...
  assert.match(controls, /\.app-shell\.theme-dark \{[\s\S]+--primary-deep: #8dc3ff;[\s\S]+--ink-strong: #e7eef4;[\s\S]+--charcoal: #a9bac7;[\s\S]+--text-muted: #98aab7;[\s\S]+--surface-soft: #17232d;/)
  assert.doesNotMatch(evaluation, /#(?:e7eef4|a9bac7|98aab7|17232d|8dc3ff)\b/i)
  assert.match(evaluation, /color: var\(--ink-strong\);/)
  assert.match(evaluation, /color: var\(--charcoal\);/)
  assert.match(evaluation, /color: var\(--text-muted\);/)
...
test('workspace dark theme consumes the same semantic theme tokens without visual remapping', async () => {
  ...
  assert.match(workspace, /color: var\(--ink-strong\);/)
  assert.match(workspace, /color: var\(--charcoal\);/)
  assert.match(workspace, /color: var\(--text-muted\);/)
...
test('history dark theme reuses surface, text, and primary theme tokens without remapping', async () => {
  ...
  assert.match(controls, /\.app-shell\.theme-dark \{[\s\S]+--primary-deep: #8dc3ff;[\s\S]+--ink-strong: #e7eef4;[\s\S]+--text-muted: #98aab7;[\s\S]+--surface-soft: #17232d;/)
  assert.doesNotMatch(history, /#(?:e7eef4|98aab7|17232d|8dc3ff)\b/i)
  assert.match(history, /color: var\(--ink-strong\);/)
  assert.match(history, /color: var\(--text-muted\);/)
...
test('shell and shared controls consume dark theme tokens while keeping one source definition', async () => {
  ...
  assert.match(shell, /color: var\(--ink-strong\);/)
  assert.match(shell, /color: var\(--charcoal\);/)
  assert.match(shell, /color: var\(--text-muted\);/)
