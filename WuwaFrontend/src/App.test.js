...
test('workspace hero centers metrics and shows separate corner deltas', async () => {
  ...
  assert.match(workspaceSource, /const sessionEchoDelta = ref\(0\)/)
  assert.match(workspaceSource, /const sessionSampleDelta = ref\(0\)/)
  ...
  assert.match(appSource, /class="hero-stat hero-stat-with-delta"[\s\S]+本次新增 \$\{sessionEchoDelta\} 个声骸[\s\S]+>\+\{\{ sessionEchoDelta \}\}/)
  assert.match(appSource, /class="hero-stat hero-stat-with-delta"[\s\S]+本次录入新增 \$\{sessionSampleDelta\} 条样本[\s\S]+>\+\{\{ sessionSampleDelta \}\}/)
