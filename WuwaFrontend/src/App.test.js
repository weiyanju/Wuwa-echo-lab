...
test('topbar uid chip shows the bound game account uid without local quick switching', async () => {
  ...
  assert.match(uidSetupSource, /import \{ normalizePlayerUid, isValidPlayerUid \} from '\.\.\/\.\.\/services\/playerUid'/)
...
test('milestone 3 uses account login and locks workbench until default game uid is bound', async () => {
  ...
  assert.match(workspaceStyleSource, /\.locked-workbench \{/)
  assert.match(workspaceStyleSource, /\.uid-binding-form \{/)
  assert.match(workspaceStyleSource, /\.uid-setup-card \{[\s\S]+grid-template-columns: 240px minmax\(0, 1fr\);/)
  assert.match(workspaceStyleSource, /\.uid-binding-form \{[\s\S]+padding: 38px 42px;/)
})
...
test('locked uid binding state shows a focused setup page without workbench chrome', async () => {
  ...
  assert.match(appSource, /<label class="uid-binding-field">\s+UID\s+<input/)
  ...
  assert.match(workspaceStyleSource, /\.uid-setup-topbar \{[\s\S]+width: 100%;/)
  assert.match(workspaceStyleSource, /\.uid-setup-topbar \{[\s\S]+margin: 0 auto;/)
  assert.match(workspaceStyleSource, /\.uid-setup-shell \{[\s\S]+width: min\(1360px, calc\(100% - 64px\)\);/)
  assert.match(workspaceStyleSource, /\.uid-setup-content \{[\s\S]+padding-top: clamp\(48px, 8vh, 88px\);/)
  assert.match(workspaceStyleSource, /\.uid-setup-card \{[\s\S]+width: min\(700px, 100%\);/)
  assert.match(workspaceStyleSource, /\.uid-setup-media \{[\s\S]+radial-gradient/)
  assert.match(workspaceStyleSource, /@media \(max-width: 860px\)[\s\S]+\.uid-setup-card \{ grid-template-columns: 1fr; \}/)
  assert.match(workspaceStyleSource, /\.disabled-tabs button \{/)
  assert.match(workspaceStyleSource, /\.disabled-tabs button\.active \{[\s\S]+background: rgba\(23, 105, 210, 0\.08\);/)
