...
test('milestone 3 uses account login and locks workbench until default game uid is bound', async () => {
  ...
  assert.match(workspaceStyleSource, /\.uid-setup-shell \.locked-workbench \{[\s\S]+width: min\(560px, 100%\);/)
  assert.match(workspaceStyleSource, /\.uid-binding-form \{[\s\S]+grid-template-columns: minmax\(0, 1fr\) 112px;/)
})
...
test('locked uid binding state shows a focused setup page without workbench chrome', async () => {
  ...
  assert.match(workspaceStyleSource, /\.locked-workbench \{[\s\S]+max-width: 560px;/)
  assert.match(workspaceStyleSource, /\.locked-workbench \{[\s\S]+padding: 28px 32px;/)
  assert.match(workspaceStyleSource, /\.uid-setup-content > \.error-text \{[\s\S]+width: min\(560px, 100%\);/)
