...
test('topbar renders the shared uid switcher for game account selection', async () => {
  const appSource = await readFile(new URL('./App.vue', import.meta.url), 'utf8')
  const uidBindingSource = await readFile(new URL('./features/auth/UidBindingPanel.vue', import.meta.url), 'utf8')
  const shellStyleSource = await readFile(new URL('./styles/shell.css', import.meta.url), 'utf8')

  assert.match(uidBindingSource, /import \{ validateUidBinding \} from '\.\/uidBinding\.js'/)
...
test('milestone 3 uses account login and locks workbench until default game uid is bound', async () => {
  ...
  assert.match(appSource, /<LoginView[\s\S]+v-else-if="!user \|\| gameAccount\.workspaceLocked\.value"[\s\S]+:view="user \? 'uid' : 'auth'"[\s\S]+:uid-busy="uidFlowBusy"[\s\S]+:error="error"[\s\S]+@submit="submitAuth"[\s\S]+@bind="submitUidBinding"[\s\S]+@clear-error="error = ''"[\s\S]+@sign-out="signOut"/)
  ...
  assert.match(uidBindingSource, /@submit\.prevent="submitUidBinding"/)
  ...
  assert.match(appSource, /async function signOut\(\) \{\s+error\.value = ''\s+await auth\.signOut\(\)/)
...
test('locked uid binding state stays inside the unchanged login page', async () => {
  const appSource = await readFile(new URL('./App.vue', import.meta.url), 'utf8')
  const loginSource = await readFile(new URL('./features/auth/LoginView.vue', import.meta.url), 'utf8')
  const uidBindingSource = await readFile(new URL('./features/auth/UidBindingPanel.vue', import.meta.url), 'utf8')

  assert.match(appSource, /<LoginView[\s\S]+:view="user \? 'uid' : 'auth'"/)
  assert.match(loginSource, /class="terminal-home"/)
  assert.match(loginSource, /class="terminal-hero-content"/)
  assert.match(loginSource, /class="terminal-auth-card"/)
  ...
  assert.match(uidBindingSource, /<header class="terminal-uid-header">\s*<h2>绑定 UID<\/h2>\s*<\/header>/)
  assert.match(uidBindingSource, /<\/header>\s*<p class="terminal-uid-intro">首次进入需要绑定一个游戏账号。<\/p>/)
  assert.doesNotMatch(uidBindingSource, /绑定游戏 UID|terminal-uid-title/)
  assert.match(uidBindingSource, /首次进入需要绑定一个游戏账号。/)
  assert.match(uidBindingSource, /placeholder="输入你的 UID"/)
  assert.match(uidBindingSource, /BIND_AND_ENTER\(\)/)
  assert.match(appSource, /<section v-else class="dashboard">/)
  assert.doesNotMatch(appSource, /<UidSetupView|uid-setup-shell|disabled-tabs|locked-workbench/)
})
