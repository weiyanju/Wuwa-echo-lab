...
test('global styles import shared tokens and base rules', async () => {
  ...
  const designSystem = await readFile(new URL('../../docs/web-ui-design-system-v2.md', import.meta.url), 'utf8')
  const interfacePrinciples = await readFile(new URL('../../docs/product-interface-principles.md', import.meta.url), 'utf8')
  const activeFontDocs = `${designSystem}\n${interfacePrinciples}`

  assert.match(entry, /@import '\.\/styles\/tokens\.css';/)
  assert.match(entry, /@import '\.\/styles\/base\.css';/)
  assert.equal(packageJson.dependencies['@fontsource/noto-sans-sc'], '^5.2.9')
  assert.match(tokens, /url\("@fontsource\/noto-sans-sc\/files\/noto-sans-sc-chinese-simplified-400-normal\.woff2"\) format\("woff2"\)/)
  assert.match(tokens, /url\("@fontsource\/noto-sans-sc\/files\/noto-sans-sc-chinese-simplified-500-normal\.woff2"\) format\("woff2"\)/)
  assert.match(tokens, /url\("@fontsource\/noto-sans-sc\/files\/noto-sans-sc-chinese-simplified-700-normal\.woff2"\) format\("woff2"\)/)
  assert.doesNotMatch(tokens, /@fontsource\/noto-sans-sc\/files\/noto-sans-sc-chinese-simplified-(100|200|300|600|800|900)-normal\.woff2/)
  assert.doesNotMatch(tokens, /@fontsource\/noto-sans-sc\/files\/[^"')]+\.woff["')]/)
  assert.match(tokens, /:root \{/)
  assert.match(tokens, /@font-face \{[\s\S]+font-family: "Wuwa CJK";[\s\S]+font-weight: 400;[\s\S]+font-display: swap;/)
  assert.match(tokens, /@font-face \{[\s\S]+font-family: "Wuwa CJK";[\s\S]+font-weight: 500;[\s\S]+font-display: swap;/)
  assert.match(tokens, /@font-face \{[\s\S]+font-family: "Wuwa CJK";[\s\S]+font-weight: 700;[\s\S]+font-display: swap;/)
  assert.match(tokens, /--font-cjk: "Wuwa CJK", "Microsoft YaHei UI", system-ui, sans-serif;/)
  assert.match(tokens, /--font-ui: var\(--font-cjk\);/)
  assert.match(tokens, /--font-title: var\(--font-cjk\);/)
  assert.match(tokens, /--font-data: var\(--font-cjk\);/)
  assert.doesNotMatch(tokens, /local\(/)
  assert.match(tokens, /--font-mono: ui-monospace, Consolas, "Courier New", monospace;/)
  assert.doesNotMatch(activeFontDocs, /local font|local\(|public\/fonts/)
  assert.match(activeFontDocs, /@fontsource\/noto-sans-sc/)
  assert.doesNotMatch(tokens, /@font-face \{[\s\S]+font-family: "Wuwa CJK";[\s\S]+font-weight: (300|600|800|900);/)
  assert.match(tokens, /--text-metric-xl: 2\.625rem;/)
  assert.match(tokens, /--weight-metric-xl: 800;/)
