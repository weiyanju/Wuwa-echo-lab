...
- 数据库密码、Django `SECRET_KEY`、远端主机、SSH 用户、私钥路径和生产域名必须通过环境变量或不入库的本地配置提供。
- `settings.py`、启动脚本、测试脚本和文档示例不得提交真实密码、真实远端地址或用户本地绝对路径。
- Django 数据库配置不能把本地密码写成代码级默认值；本地启动工具如需便利值，也必须明确标记为仅本地，并在进入共享或生产环境前强制覆盖。
- `DEBUG=True`、固定 `ALLOWED_HOSTS` 和本地 CORS/CSRF 来源只适用于开发环境。部署前必须拆分或参数化生产设置。
- README 中的账号、主机和密码只能使用占位值，不能成为隐式生产承诺。

当前仓库仍处于 pre-release 本地开发阶段。任何云端部署工作开始前，必须先完成配置外部化与安全检查。
