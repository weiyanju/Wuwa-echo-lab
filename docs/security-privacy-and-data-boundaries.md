...
外部本地识别客户端只通过窗口检测、截图和本地离线 OCR 获取用户可见信息。
...
- 系统账号由后端管理。
- Web 和外部本地识别客户端使用后端认证结果。
- 普通用户 UI 不展示后端地址。
- 密码不写入诊断日志。
- token、cookie、session 信息不写入普通日志。
- 登录失败提示不应泄露额外信息，除非后端已明确设计。
- 注册入口恢复未完成开户前必须重新校验系统账户密码；用户名存在本身不能建立 session。
- 停用账户与凭据错误使用同一拒绝路径，不得建立 session 或泄露开户完成状态。
- 只有不存在任何已绑定 UID 的账户可以从注册入口恢复；已完成账户必须使用登录入口。
- 注册恢复不得覆盖密码、创建重复用户或重复默认 `GameAccount`，也不得记录凭据、cookie 或 session。

未来认证方式变化时，必须同时评估 Web 调用方式、外部本地识别客户端调用方式、CSRF 或 token 策略、本地保存内容、登出和过期行为。

### 3.1 开发与部署配置

仓库中的默认配置只服务本地开发，不能直接视为可部署配置。

规则：

- 当前唯一允许写入受版本控制文件的凭据型默认值，是 pre-release 阶段仅绑定本机回环 PostgreSQL 的开发密码 `root`。这是用户明确批准的临时开发约定，不是生产凭据，也不得复用于远端、共享或生产数据库。
- 本地开发默认使用数据库 `wuwa_dev`、角色 `PostgreSQL`、主机 `127.0.0.1` 和端口 `5432`。自动化协作者应使用这条 PostgreSQL 路径，不得为了规避配置而切换到 SQLite。
- `start-dev.bat` 只负责本机 PostgreSQL；不得在其中保存或恢复 SSH 隧道、真实远端地址、SSH 用户、私钥路径或其他部署专用配置。
- 除上述本地密码例外外，数据库密码、Django `SECRET_KEY`、远端主机、生产域名及其他私有配置必须通过环境变量或不入库的本地配置提供。
- 受版本控制的设置、脚本、测试和文档不得包含真实生产密码、真实远端地址或用户本地绝对路径。

生产配置以 `WUWA_ENV=production` 为明确切换门槛：

- 必须显式提供 `DJANGO_SECRET_KEY`、`DB_PASSWORD` 和逗号分隔的 `DJANGO_ALLOWED_HOSTS`；缺少任一项时拒绝启动。
...
独立客户端仓库负责窗口检测、截图、OCR、本地缓存和诊断实现；Wuwa 继续负责认证、ownership、识别写入与回滚安全。
...
本仓库不得记录或提交客户端私有地址、用户路径、cookie、session、截图或日志。
...
- 是否改变了 OCR 隐私边界？
- 是否有测试覆盖越权和回滚？
- 是否把密码、远端主机、私钥路径或生产配置写入了受版本控制文件？
